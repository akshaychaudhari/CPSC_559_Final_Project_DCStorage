const DCStorage = artifacts.require('./DCStorage.sol')

require('chai')
  .use(require('chai-as-promised'))
  .should()

contract('DCStorage', ([deployer, uploader]) => {
  let dcstorage

  before(async () => {
    dcstorage = await DCStorage.deployed()
  })

  describe('deployment', async () => {
    it('deploys successfully', async () => {
      const address = await dcstorage.address
    ...
    it('has a name', async () => {
      const name = await dcstorage.name()
      assert.equal(name, 'DCStorage')
    })
  })
  ...
  describe('file', async () => {
    ...
    before(async () => {
      result = await dcstorage.uploadFile(fileHash, fileSize, fileType, fileName, fileDescription, { from: uploader })
      fileCount = await dcstorage.fileCount()
    })
    ...
    it('upload file', async () => {
      ...
      await dcstorage.uploadFile('', fileSize, fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have size
      await dcstorage.uploadFile(fileHash, '', fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await dcstorage.uploadFile(fileHash, fileSize, '', fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await dcstorage.uploadFile(fileHash, fileSize, fileType, '', fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have description
      await dcstorage.uploadFile(fileHash, fileSize, fileType, fileName, '', { from: uploader }).should.be.rejected;
    })

    //check from Struct
    it('lists file', async () => {
      const file = await dcstorage.files(fileCount)
